...
class Ssv1(torch.utils.data.Dataset):
    ...
    def __init__(self, cfg, mode, num_retries=10):
        """
        Load Something-Something V1 data (frame paths, labels, etc. ) to a given
        ...
        Please see DATA.md for more information about the data format.
        Args:
            cfg (CfgNode): configs.
    ...
    def _construct_loader(self):
        ...
        label_file = os.path.join(
            self.cfg.DATA.PATH_TO_DATA_DIR,
            "something-something-v1-{}.csv".format(
        ...
        for video in label_csv:
            # video_name = video["id"]
            # template = video["template"]
            # template = template.replace("[", "")
            # template = template.replace("]", "")
            video_name, template = video.split(";")
